...
合法输出只有 `0` 到 `9`，所以当上升沿到来、`slowena = 1`，并且读取到 `q = 9` 时，就必须把 `q` 的下一状态写成 `0`：
...
因为当 `q = 9` 时，`q >= 10` 为假，程序会执行加一：

```verilog
q <= q + 4'd1;
```

于是 `q` 的下一状态会变成 `10`，计数序列就会错误地出现：
...
> 判断的是“当前允许出现的最大值”，不是“最大值再加一”。

对于 0～9 计数器，最大允许值是9，所以判断9；对于 1～10 计数器，最大允许值是10，所以判断10。
...
这里的 `<=` 会被当作“小于等于”比较。由于正常计数时 `q` 几乎一直小于等于9，这样会导致计数器不断清零。条件推荐写成：

```verilog
if (q >= 4'd9)
```

如果能够保证 `q` 永远不会进入非法状态，也可以写：
...
```text
if (q >= 4'd9)   条件判断：q 是否到达上限
q <= 4'd0;       非阻塞赋值：把 q 的下一状态写成0
```
...
- 0～9 计数器应在读取到最大合法值9时回到0，不能等到10才回零。
- 条件判断中的 `>=` 和非阻塞赋值中的 `<=` 含义不同，不能因为符号相似而混淆。
