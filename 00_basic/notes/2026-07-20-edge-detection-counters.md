...
- `in` 表示当前输入。
- `in_d` 表示上一个时钟周期保存的输入。
- 每个 `clk` 上升沿到来时，触发器对当时已经稳定的 `in` 进行采样，这个值可以称为“当前采样值”。

这里的“当前”不是指上升沿之后过了一段时间才出现的新值，而是指触发器在上升沿处采样到的值。在真实硬件中，`in` 应当在上升沿前满足建立时间，并在上升沿后满足保持时间，因此它需要在上升沿附近保持稳定。
...
对这段代码，更准确的理解不是“所有右侧都使用上一周期的值”，而是：

> 在本次时钟上升沿触发的过程块中，所有 `<=` 的右侧表达式都先根据寄存器更新前的现有信号值进行计算；计算完成后，左侧寄存器再统一更新。

因此，上下两行中的 `in` 是同一个值，都是本次时钟上升沿采样到的当前输入：
...
- `in` 是外部输入，没有在这个 `always` 块中被赋值，因此两行读取到的是同一个当前输入值。
- `in_d` 是寄存器，并且第一行正在安排它更新。
- 因为使用非阻塞赋值 `<=`，第一行不会立刻修改 `in_d`。
- 所以第二行计算时，`in_d` 仍然是上一个时钟周期保存的输入。

例如，某个时钟上升沿到来时，更新前的信号为：
...
```text
新 in_d  = 当前 in
新 pedge = 当前 in & ~旧 in_d
```

这里所谓“`<=` 同时执行”，主要是指左侧寄存器在时钟沿后的同一更新阶段改变，而不是指所有右侧信号都必须是上一周期值。
...
第一行会立刻修改 `in_d`，第二行就会使用已经被改成当前输入的 `in_d`。例如 `in = 1` 时，第二行会变成 `1 & ~1 = 0`，从而无法正确检测上升沿。
...
- `begin...end` 只负责组合语句，非阻塞赋值 `<=` 会让右侧先按更新前的现有值计算，再让左侧寄存器统一更新。
- 在边沿检测代码中，两行读取的是同一个当前 `in`，而 `in_d` 在本次更新前仍保存上一周期的输入。
